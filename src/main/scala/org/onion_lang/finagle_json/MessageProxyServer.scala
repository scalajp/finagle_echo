...
object MessageProxyServer {
  ...
  class LivingClientManagementService(clientConnection: ClientConnection) extends Service[(ChannelHandlerContext, JsonAST.JValue), JsonAST.JValue] {self =>
    private var context: ChannelHandlerContext = null
    def makeAck(success: Boolean): String = <t>{{ "type":"ack", "success":{success} }}</t>.text
    def processBroadcast(request: JsonAST.JValue): JsonAST.JValue = {
      ...
      for((ctx, dev) <- contexts.collect{ case ((ch, dev), ctx) if ch == channel => (ctx, dev)}) {
      ...
      try {
        transaction {
          val cls = from(clients)(c =>
            ...
            select(c)
          ).toList.sortBy{c => - c.timestamp.value.getTime()}
          cls match {
            case Nil =>
    ...
    def processRequest(request: JsonAST.JValue): JsonAST.JValue = {
      ...
      (for(ctx <- contexts.get((channel, destinationDeviceId))) yield {
        val msgJson =  <t>{{"type":"response", "message":"{message}", "channel":"{channel}", "source_device_id":"{sourceDeviceId}", "destination_device_id":"{destinationDeviceId}"}}</t>.text
        val channelFuture = Channels.succeededFuture(ctx.getChannel())
